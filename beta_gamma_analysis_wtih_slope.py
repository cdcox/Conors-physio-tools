...
def calculate_slope(start,end,voltage,i,filen,sampling_rate):
    scale_factor = int(sampling_rate/1000)
    range_of_hunt = int((end - start)/10) #find 10% to knock it off
    range_KO = range_of_hunt*3
    voltage_values = voltage[start:end-range_KO]
    steps = (end-start)-range_KO
    line_Results = linregress(np.linspace(0,steps/scale_factor,steps),voltage_values)
    slope = line_Results[0]
    plt.plot(voltage[start:end-range_KO])
    plt.savefig(os.path.join(out_dir,filen+str(i)+'.jpg' ))
    plt.cla()
    plt.clf()
    if (line_Results[2])**2<0.5:
...
#for debugging
#directory = r'C:\Users\cdcox_000\Documents\test'
#out_dir = r'C:\Users\cdcox_000\Documents\test\out'
#sampling_rate = 10000
#seconds_baseline = 0.001
#frquency = 10
#ppoints_to_check = 110
...
for filen in file_list:
    ...
    for i in range(1,ender+1):
        ...
        per_burst_slope_start = int((i-1)*fire_rate+first_stim+slope_gap)
        slope = calculate_slope(per_burst_slope_start,voltage_min_target,voltage,i,filen,sampling_rate)
