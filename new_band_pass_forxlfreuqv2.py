...
for book_name in dir_list:
    ...
    for thresholds in [-0.1,-0.05,-0.025,-0.01]:
        for sheet_name in snames:
            ...
            for cols in range(sheet.ncols):
                ...
                for jjj in range(epochs-1):
                    ...
                    starts_stops = generate_starts_stops(epoched,thresholds)
                    seconds = len(bb_filt_out[:])/fs
                    if len(starts_stops)==0:
                        out_hist=list(np.zeros(np.round(seconds).astype(int)))
                    else:
                        FF,out_hist = reverberation_test(starts_stops)
                        FF=np.round(FF,decimals=2)
                        ...
                        '''
                    all_out_names.append(book_name+sheet_name+str(j)+'_'+str(i)+names+'_'+str(jjj)+'_'+str(FF))
                    ...
                    plt.clf()
                    '''
                    outputter[thresholds][book_name+'_'+sheet_name+str(j)+'_'+str(i)+names+'_'+str(jjj)+'_'+str(FF)]=[names,out_hist]
...
for nt in new_threshs:
    for k in ['norm','orig']:
        ...
        sheetout=out_book.add_sheet('sheet_'+k+str(nt))
        skeys=list(out_thresh.keys())
        skeys.sort()
        for kn,nameout in enumerate(skeys):
            temp_out_hist=out_thresh[nameout]
            sheetout.write(kn,0,nameout)
            temp_out_hist=np.array(temp_out_hist[1]).astype(float)
            if k=='norm' and np.sum(temp_out_hist)!=0:
                temp_out_hist=temp_out_hist/np.sum(temp_out_hist)
                print(temp_out_hist)
            for knn in range(len(temp_out_hist)):
                sheetout.write(kn,knn+1,temp_out_hist[knn])
out_book.save(os.path.join(directory,'total_hist_out_all_thresh1.xls'))
